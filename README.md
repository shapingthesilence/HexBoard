...
- firmware-backed synth wavetable upload, download, list, erase, and Serum
  ...
  four-level fixed `32 x 512` mipmapped byte format, stores them by
...
Firmware implementation is grouped by owner under `src/firmware/`; subsystem headers expose cross-module APIs, while implementation details stay in the owning `.cpp`. Tuning and Dynamic JI live in `src/firmware/tuning/`, layout/scale/palette/preset models in `src/firmware/model/`, stable board constants in `src/firmware/hardware/HardwareConfig.h`, grid/input/LED hardware in `src/firmware/hardware/`, storage schemas in `src/firmware/storage/PersistentDataModels.h`, settings/profile/preset-sync persistence in `src/firmware/storage/`, built-in synth waveforms in `src/firmware/synth/BuiltinWavetables.cpp`, synth/audio runtime in `src/firmware/synth/SynthAudio.cpp`, and OLED/GEM menu code in `src/firmware/menu/`.

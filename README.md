...
The firmware modules are grouped by owner under `src/firmware/`; subsystem headers expose cross-module APIs, while implementation details stay in the owning `.cpp`. Platform helpers, runtime defaults, diagnostics/timing, tuning and Dynamic JI, grid state, MIDI transport/routing/input/delegated-control helpers, pitch assignment, command buttons, LED animations, built-in synth waveforms, and menu modules now compile as standalone modules. Tuning and Dynamic JI live in `src/firmware/tuning/`, layout/scale/palette/preset models live in `src/firmware/model/`, stable board dimensions and pin assignments live in `src/firmware/hardware/HardwareConfig.h`, grid state and input hardware live in `src/firmware/hardware/`, storage schemas live in `src/firmware/storage/PersistentDataModels.h`, settings/profile persistence, synth preset/wavetable storage, and preset-sync object handling live in `src/firmware/storage/`, built-in synth waveforms live in `src/firmware/synth/BuiltinWavetables.cpp`, and OLED/GEM menu code is split across `MenuAndDisplay`, `PlayedNotesOverlay`, `SynthPresetMenu`, and `SynthWavetableMenu` under `src/firmware/menu/`.

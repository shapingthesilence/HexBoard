...
build/HexBoard.ino.uf2: $(FIRMWARE_SOURCES) Makefile | build
	arduino-cli compile -b $(FQBN) $(BUILD_PROPERTIES) --output-dir build .
...
install: build/HexBoard.ino.uf2 /run/media/*/RPI-RP2/INFO_UF2.TXT
	echo "Trying to copy into mounted device"
	cp build/HexBoard.ino.uf2 /run/media/*/RPI-RP2/
	echo "Installed."
	sleep 7
	echo "Rebooted."

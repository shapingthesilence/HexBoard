...
- Modules still included through `FirmwareUnity.cpp` keep their `#if HEXBOARD_FIRMWARE_UNITY` guard before library includes. Standalone modules such as `Tuning.cpp`, `Layout.cpp`, `ScalePalettePreset.cpp`, `PersistentDataModels.cpp`, `Settings.cpp`, `SynthPresetStorage.cpp`, `SynthWavetableStorage.cpp`, `PresetSyncProtocol.cpp`, `PresetSyncGeometry.cpp`, `PresetSyncSynthObjects.cpp`, and `PresetSync.cpp` are omitted from `FirmwareUnity.cpp` and compile normally with direct headers.

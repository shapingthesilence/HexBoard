...
- built-in tuning, layout, and scale catalogs are exposed as generated read-only
  ...
  `/layouts.dat`; the on-device geometry browsers suppress the `/Built In`
  folder label for factory entries
...
4. Verify the fixed-memory geometry browsers filter linked layouts/scales correctly and key spinner logic still behaves

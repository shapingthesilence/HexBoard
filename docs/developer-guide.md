...
- Modules still included through `FirmwareUnity.cpp` keep their `#if HEXBOARD_FIRMWARE_UNITY` guard before library includes. Standalone modules such as `PlatformCommon.cpp`, `RuntimeDefaults.cpp`, `DiagnosticsTiming.cpp`, `MidiTransport.cpp`, `MidiRouting.cpp`, `MidiInput.cpp`, `DelegatedControl.cpp`, `ExternalMidiLedState.cpp`, `Tuning.cpp`, `Layout.cpp`, `ScalePalettePreset.cpp`, `PitchAssignment.cpp`, `LedAnimations.cpp`, `PersistentDataModels.cpp`, `Settings.cpp`, `SynthPresetStorage.cpp`, `SynthWavetableStorage.cpp`, `PresetSyncProtocol.cpp`, `PresetSyncGeometry.cpp`, `PresetSyncSynthObjects.cpp`, and `PresetSync.cpp` are omitted from `FirmwareUnity.cpp` and compile normally with direct headers.
...
- `src/firmware/hardware/`: grid state, command buttons, scan/rotary handling, LED rendering, and standalone LED animations

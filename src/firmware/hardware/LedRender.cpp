...
void setLEDcolorCodes() {
  // ---- Diatonic MOS preparation (runs once per color refresh) ----
  // Layer 0 = diatonic naturals (white), positive layers = sharp side
      ...
      if (userGeometryRuntime.active && userGeometryRuntime.paletteActive && colorMode == CUSTOM_COLOR_MODE) {
        ...
        case PIANO_INCANDESCENT_COLOR_MODE:
          ...
          case DIATONIC_COLOR_MODE:
            {
            uint16_t rawIndex = paletteIndex;
            if (!mosValid) {
              ...
              int k = rawIndex - lowerPos;
              int kFromUpper = upperPos - rawIndex;
              bool equi = k != 0 && k == kFromUpper;
              int layer = k == 0 ? 0 : (k <= kFromUpper ? k : -kFromUpper);
              if (layer == 0) {

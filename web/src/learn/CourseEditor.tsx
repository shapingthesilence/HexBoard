...
import {compatibilityReport,transposeCourseLayout,incompatibleCourseTuning,replaceCourseBundle} from "./courseStructure.ts";
...
export function CourseEditor({ initial, bundles, transport, connected, onSave, onClose, renderPreview }: {
  ...
  const [pendingBundle,setPendingBundle]=useState<TuningBundle>();
  const [confirmClear,setConfirmClear]=useState(false);
  const [selectorVersion,setSelectorVersion]=useState(0);
  const [transposeLayoutId,setTransposeLayoutId]=useState(initial.bundle.activeLayoutIdHex);
  ...
  const warnings=compatibilityReport(draft).filter(row=>row.issues.length);
  const transposeLayout=draft.bundle.layouts.find(item=>item.objectIdHex===transposeLayoutId)??draft.bundle.layouts[0];
  function applyBundle(bundle:TuningBundle,clear=false){setDraft(replaceCourseBundle(draft,bundle,clear));setLayoutId(bundle.activeLayoutIdHex);setTransposeLayoutId(bundle.activeLayoutIdHex);setVoice(undefined);setStep(0);setPendingBundle(undefined);setConfirmClear(false);setSelectorVersion(value=>value+1);}
  ...
  return <section className="learnCard courseEditor" aria-label="Course editor">
    ...
    {settingsOverlay==="course"&&<div className="modalOverlay" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setSettingsOverlay(undefined);}}><section className="modalPanel courseSettingsDialog" role="dialog" aria-modal="true" aria-labelledby="course-settings-title"><header className="learnPracticeHeader"><h3 id="course-settings-title">Course Settings</h3><button type="button" onClick={()=>setSettingsOverlay(undefined)}>Close</button></header>
      ...
      <CourseInstrumentSelector key={selectorVersion} current={draft.bundle} bundles={bundles} transport={transport} connected={connected} onApply={bundle=>{if(incompatibleCourseTuning(draft,bundle)){setPendingBundle(bundle);setConfirmClear(false);}else applyBundle(bundle);}}/>
      {warnings.length===0&&transpositionControls}
      {warnings.length>0&&<details className="learnSettings learnWarning courseCompatibility"><summary>⚠ Layout compatibility · {warnings.length} lesson/layout {warnings.length===1?"needs":"pairs need"} attention</summary><p>Transpose a layout or revise its notes and key assignments. You can still save.</p>{transpositionControls}<ul>{warnings.map(row=><li key={`${row.lessonId}:${row.layoutId}`}>{row.lesson} · {row.layout}: {row.issues.join("; ")}</li>)}</ul></details>}
      {pendingBundle&&<div className="modalOverlay"><section role="alertdialog" aria-modal="true" aria-labelledby="tuning-warning-title" className="modalPanel"><h3 id="tuning-warning-title">{confirmClear?"Are you sure?":"Incompatible Tuning: Clear Notes/Buttons/Fingers?"}</h3><p>{confirmClear?"This clears notes, buttons, and fingers from every practice lesson. Lesson titles and content pages will remain.":"The selected tuning cannot play all the notes already in this course."}</p><div className="learnActions"><button autoFocus key={confirmClear?"confirm-revert":"revert"} onClick={()=>{setPendingBundle(undefined);setConfirmClear(false);setSelectorVersion(value=>value+1);}}>Revert</button><button onClick={()=>{if(confirmClear)applyBundle(pendingBundle,true);else setConfirmClear(true);}}>{confirmClear?"Clear notes, buttons & fingers":"Clear"}</button></div></section></div>}

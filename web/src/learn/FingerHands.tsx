...
export function FingerHands({cue,disabled,onChoose}:{cue?:KeyCue;disabled:boolean;onChoose:(hand:"left"|"right",finger:number)=>void}) {
  return <svg className="courseHands" viewBox="0 0 340 240" aria-label="Recommended finger selection">
    {(["left","right"] as const).map((hand,index)=><g key={hand} transform={`translate(${index*175} 0)`}>
      ...
      {fingers.map(finger=>{const x=hand==="left"?finger.x:140-finger.x,angle=hand==="left"?finger.angle:-finger.angle;const selected=cue?.hand===hand&&cue.finger===finger.number;
        const choose=()=>{if(!disabled)onChoose(hand,finger.number);};
        return <g key={finger.number} role="button" aria-label={`${hand==='left'?'Left':'Right'} ${finger.name} (${finger.number})`} aria-pressed={selected} aria-disabled={disabled} tabIndex={disabled?-1:0} className={`handFinger ${selected?'selected':''}`} onClick={choose} onKeyDown={event=>{if(['Enter',' '].includes(event.key)){event.preventDefault();choose();}}}>
          <rect x={x-10} y={finger.y} width={20} height={finger.height} rx={10} transform={`rotate(${angle} ${x} ${finger.y+finger.height})`}/>
          <text x={x} y={finger.y+18} textAnchor="middle" transform={`rotate(${angle} ${x} ${finger.y+finger.height})`}>{finger.number}</text>
        </g>;
      })}

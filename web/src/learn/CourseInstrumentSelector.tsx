...
export function CourseInstrumentSelector(props: InstrumentProps) {
  ...
  return <><button type="button" onClick={()=>setOpen(true)}>Tuning &amp; layouts…</button>
    {open&&<dialog ref={dialog} className="courseLibraryDialog" aria-labelledby="course-library-heading" onCancel={()=>setOpen(false)} onClose={()=>setOpen(false)}>
      <header className="learnPracticeHeader"><h2 id="course-library-heading">Tuning &amp; layouts</h2><button type="button" onClick={()=>setOpen(false)} aria-label="Close tuning selection">Close</button></header>
...
function InstrumentChoices({current,requiredLayout,bundles,transport,connected,onApply}: InstrumentProps) {
  ...
  async function changeSource(value:string){
    const request=++generation.current; setSource(value);setError("");setTuningHandle("");setSelectedLayouts([]);setRequired("");
  ...
  async function chooseTuning(value:string){
    setSelectedLayouts([]);setRequired("");setError("");
